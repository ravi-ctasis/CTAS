    ...
    <PageShell>
      ...
      <main>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 max-w-3xl mx-auto">
                {[
                  ...
                  <motion.div
                    ...
                    className="rounded-2xl bg-white/[0.05] border border-white/10 p-4 flex items-center justify-center gap-3"
                  >
        ...
        <PageFilterSidebar
          ...
          searchPlaceholder="Search case studies, industries..."
          showClear={
          ...
          resultCount={{ showing: sortedCaseStudies.length, total: CASE_STUDIES.length }}
          sortControl={
            <select
              value={sortBy}
              onChange={(e) => setSortBy(e.target.value)}
              className="px-4 py-2.5 rounded-xl border border-slate-200 bg-white text-sm focus:outline-none focus-visible:ring-2 focus-visible:ring-[#249BCA]"
          ...
          {featuredStudies.length > 0 && (
            <div className="mb-8">
              <div className="flex items-center gap-3 mb-5">
                ...
                <h2 className={`${fraunces.className} text-xl sm:text-2xl font-medium text-slate-900`}>Featured</h2>
              </div>
                ...
                {featuredStudies.map((study, i) => {
                  ...
                  return (
                    <motion.article
                      ...
                      className={`grid lg:grid-cols-2 gap-0 rounded-[1.25rem] border border-slate-200/80 bg-white overflow-hidden hover:shadow-xl transition-shadow ${
                        i % 2 === 1 ? "lg:[direction:rtl]" : ""
                      }`}
                    >
                      <div className="relative aspect-[16/10] lg:aspect-auto lg:min-h-[320px] lg:[direction:ltr]">
                        <Image
                          src={study.image}
                          alt={study.title}
                          fill
                          ...
                          priority={i === 0}
                        />
                        <div className="absolute top-3 left-3 flex items-center gap-2 px-2.5 py-1 rounded-full bg-white/90 text-xs font-semibold text-slate-800">
                          <IndustryIcon className="w-3.5 h-3.5" style={{ color: CYAN }} />
                          {study.industry}
                        </div>
                        <span
                          className="absolute top-3 right-3 px-2.5 py-1 rounded-full text-[10px] font-bold text-white uppercase tracking-wide"
                          style={{ backgroundColor: NAVY }}
                        >
                          Premium
                        </span>
                      </div>
                      <div className="p-5 sm:p-6 lg:p-7 flex flex-col lg:[direction:ltr]">
                        <h3 className={`${fraunces.className} text-lg sm:text-xl font-medium text-slate-900 mb-1.5`}>
                          {study.title}
                        </h3>
                        <p className="text-sm text-slate-500 font-medium mb-2">{study.subtitle}</p>
                        <p className="text-sm text-slate-600 leading-relaxed mb-3 line-clamp-3">{study.longDescription}</p>
                        <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-[#EAF3F8] text-xs font-semibold text-slate-700 mb-3 w-fit">
                        ...
                        <div className="flex flex-wrap gap-1.5 mb-3">
                          {study.technologies.slice(0, 4).map((tech) => (
                            <span
                              key={tech}
                              className="px-2 py-0.5 text-xs font-medium rounded-md bg-[#F6F8FA] text-slate-600"
                            >
                              {tech}
                            </span>
                          ))}
                        ...
                        <Link
                          href={study.href}
                          className="inline-flex items-center justify-center gap-2 px-5 py-2.5 text-sm font-semibold rounded-full text-[#08141F] transition-all hover:opacity-90 mt-auto w-fit"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
              {sortedCaseStudies.map((study, index) => (
                <motion.article
                  ...
                  className="group flex flex-col rounded-[1.25rem] border border-slate-200/80 bg-white overflow-hidden hover:shadow-lg transition-all"
                >
                  ...
                  <div className="p-4 sm:p-5 flex flex-col flex-1">
                    ...
                    <p className="text-sm text-slate-600 mb-3 line-clamp-3 flex-1">{study.description}</p>
                    <div className="flex flex-wrap gap-1.5 mb-3">
